...
dataFolder = 'CWRU_40';
srcData  = 'src_7_0';
tarData = ['FE_tar_7_1', 'FE_tar_7_2', 'FE_tar_7_3', 'FE_tar_14_1', 'FE_tar_14_2', 'FE_tar_14_3', 'FE_tar_21_1', 'FE_tar_21_2', 'FE_tar_21_3'];
...
#%%
InputDataSRC=sio.loadmat(f'../{srcData}.mat')
src_input = np.float32(np.array(InputDataSRC['Data'])) 
src_input1 = np.reshape(src_input, [src_input.shape[0],1, 10,10])
...
for mload in range(len(tarData)):
    f0 = sio.loadmat(f'../{dataFolder}/{tarData[mload]}.mat')
    InputDataTAR = f0['Y'][0,0]
    tar_input = np.float32(np.array(InputDataTAR['training_inputs']))
    tar_input1 = np.reshape(tar_input, [tar_input.shape[0],1, 10,10])
    ...
    test_tar_input1 = np.reshape(test_tar_input, [test_tar_input.shape[0],1, 10,10])
...
#%%
    ...
    for epoch in range(n_epoch):
        ...
        data_target_iter = iter(loader_tar)
    ...
    worksheet.write(mload+2,4, acc*100)
    worksheet.write(mload+2,5, (endTime-startTime))
